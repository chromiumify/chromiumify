...
switch (process.argv[2]) {
  ...
  case 'init':
    shell.exec('npm install chrome-net http-node chrome-fs chrome-dgram chrome-debug chrome-depd chrome-util-deprecate ' + args.split(' ').slice(1).join(' '))
  break
  ...
  default:
    shell.exec('node ' + __dirname + '/node_modules/browserify/bin/cmd -r chrome-net:net -r http-node:http -r chrome-fs:fs -r chrome-dgram:dgram -r chrome-debug:debug -r chrome-depd:depd -r chrome-util-deprecate:util-deprecate ' + process.argv.slice(2).join(' '))
    break
}
